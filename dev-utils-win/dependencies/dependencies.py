...
class subinfo(info.infoclass):
    def setTargets(self):
        for ver in ["1.5"]:
            ...
            self.targetInstallPath[ver] = "dev-utils-wins/dependencies/"
...
class Package(BinaryPackageBase):
    ...
    def install(self):
        if not BinaryPackageBase.install(self):
            return False
        utils.createShim(os.path.join(self.imageDir(), "dev-utils-wins", "bin", "dependencies.exe"),
                         os.path.join(self.imageDir(), "dev-utils-wins", "dependencies", "Dependencies.exe"))
        utils.createShim(os.path.join(self.imageDir(), "dev-utils-wins", "bin", "clrphtester.exe"),
                         os.path.join(self.imageDir(), "dev-utils-wins", "dependencies", "clrphtester.exe"))
        return True
