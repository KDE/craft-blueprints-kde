...
class subinfo(info.infoclass):
    def setTargets(self):
        for branch in ["master", "release"]:
            ...
            self.targetInstallPath[branch] = os.path.join("dev-utils-wins", "cmake-src")
...
class Package(CMakePackageBase):
    ...
    def install(self):
        ...
        for name in ["cmake", "cmake-gui", "cmcldeps", "cpack", "ctest"]:
            if not utils.createShim(os.path.join(self.imageDir(), "dev-utils-wins", "bin", f"{name}.exe"),
                                    os.path.join(self.imageDir(), "dev-utils-wins", "cmake", "bin", f"{name}.exe")):
                return False
        return True
