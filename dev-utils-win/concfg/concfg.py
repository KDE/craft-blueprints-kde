...
class subinfo(info.infoclass):
    def setTargets(self):
        ...
        self.targetInstallPath["master"] = "dev-utils-wins/concfg/"
...
class Package(BinaryPackageBase):
    ...
    def install(self):
        if not BinaryPackageBase.install(self):
            return False
        utils.createShim(os.path.join(self.imageDir(), "dev-utils-wins", "bin", "concfg.exe"),
                         CraftCore.cache.findApplication("powershell"),
                         args="-NoProfile {path}".format(
                             path=os.path.join(self.imageDir(), "dev-utils-wins", "concfg", "bin", "concfg.ps1")),
