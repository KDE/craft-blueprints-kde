...
class subinfo(info.infoclass):
    def setTargets(self):
        for version in ['4.0.0']:
            ...
            self.targetInstallPath[version] = f"dev-utils-wins/xsd"
...
class Package(BinaryPackageBase):
    ...
    def install(self):
        if not BinaryPackageBase.install(self):
            return False
        return utils.createShim(os.path.join(self.imageDir(), "dev-utils-wins", "bin", "xsd.exe"),
                                os.path.join(self.imageDir(), "dev-utils-wins", "xsd", "bin", "xsd.exe"))
