...
class Package(BinaryPackageBase):
    ...
    def install(self):
        ...
        return (utils.createShim(self.imageDir() / "dev-utils/bin/mvn", self.installDir() / f"bin/mvn{'.cmd' if CraftCore.compiler.isWindows else ''}", env=env) and
                utils.createShim(self.imageDir() / "dev-utils/bin/mvn", self.installDir() / f"bin/mvn{'.cmd' if CraftCore.compiler.isWindows else ''}", env=env))
