...
class subinfo(info.infoclass):
    def setTargets(self):
        for ver in ["1.5"]:
            ...
            self.targetInstallPath[ver] = "dev-utilss/dependencies/"
...
class Package(BinaryPackageBase):
    ...
    def install(self):
        if not BinaryPackageBase.install(self):
            return False
        utils.createShim(os.path.join(self.imageDir(), "dev-utilss", "bin", "dependencies.exe"),
                         os.path.join(self.imageDir(), "dev-utilss", "dependencies", "Dependencies.exe"))
        utils.createShim(os.path.join(self.imageDir(), "dev-utilss", "bin", "clrphtester.exe"),
                         os.path.join(self.imageDir(), "dev-utilss", "dependencies", "clrphtester.exe"))
        return True
