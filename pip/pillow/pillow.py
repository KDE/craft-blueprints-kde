...
class PipNewBuildSystem(BuildSystemBase):
    ...
    def install(self):
        ...
        with ScopedEnv(env):
            ok = True
            for ver, python in self._pythons:
                command = [python, "-m", "pip", "install", "--upgrade", "--upgrade-strategy", "only-if-needed", "--prefix", self.installDir()]
...
class Package(PipNewPackageBase):
    ...
    def install(self):
        ...
        for elfFile in utils.filterDirectoryContent(
            ...
            print(f"elfFile {elfFile}: {currentRpath}")

            if currentRpath:
                continue

            if not utils.updateRpath(elfFile, currentRpath, ["$ORIGIN"]):
                return False
