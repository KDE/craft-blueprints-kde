...
class subinfo(info.infoclass):
    def setTargets(self):
        ...
        self.patchToApply["d99a0db"] = [
            ("fix-perl-path.diff", 1),
            # Link intl to fix
            # squashfs-root/usr/bin/melt-7: symbol lookup error: squashfs-root/usr/lib/ladspa/alias_1407.so: undefined symbol: libintl_bindtextdomain
            ("link-intl.diff", 1),
        ]
        self.patchLevel["d99a0db"] = 1
...
class Package(AutoToolsPackageBase):
    def __init__(self, **kwargs):
        ...
        self.subinfo.options.make.args += f"PERL5LIB={self.shell.toNativePath(CraftCore.standardDirs.craftRoot())}/lib/perl5/site_perl/5.40.1"
